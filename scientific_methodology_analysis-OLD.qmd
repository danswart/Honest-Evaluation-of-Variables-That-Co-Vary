...
black_students <- c(300, 850, 1200, 1540, 1580)  # SAT scores within "Black" category
white_students <- c(400, 900, 1300, 1520, 1600)  # SAT scores within "White" category

within_group_var <- var(black_students) + var(white_students)  # Massive
between_group_var <- var(c(mean(black_students), mean(white_students)))  # Small
...
```r
# "Black students" category includes:
nigerian_immigrants <- c(1400, 1450, 1500)  # High-achieving
mixed_race_suburban <- c(1200, 1350, 1400)  # Middle-class
urban_disadvantaged <- c(300, 400, 500)     # Low-resourced

# Enormous within-group variance makes between-group comparisons invalid
within_black_variance <- var(c(nigerian_immigrants, mixed_race_suburban, urban_disadvantaged))
# This variance is often LARGER than variance between racial categories
```
...
# This means race explains only 15% of academic variance
# Yet policies are based on treating it as the dominant factor
```
...
**Instead of:**
- Comparing all Black students to all White students
- Comparing all Hispanic students to all Asian students

**Scientific approach:**
- Compare high-SES Black students to high-SES White students
- Compare recent immigrant Asian students to recent immigrant Hispanic students  
...
High-performing Black, Hispanic, White, and Asian students often have more in common with each other than with low-performing members of their own racial group.
:::
