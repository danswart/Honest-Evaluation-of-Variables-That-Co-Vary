...
set.seed(123)
# This function has perfect predictability but zero correlation/covariance
x <- -10:10
y <- x^2
plot(x, 
     y, 
     col  = "blue",    # sets point color
     pch  = 16,        # solid circle
     main = paste("Correlation = ", round(stats::cor(x, y), 2),
                   "Covariance = ", round(stats::cov(x, y), 2)
                   ),
     xlab = "x", ylab = "y")
...
# plot with the correct correlation in the title
...
### When Good R² Doesn't Mean Useful Predictions

**Example 1: High R² but Useless Predictions**

```{r}
# Simulate stock price prediction with HIGH R² but useless predictions
set.seed(123)
stock_data <- data.frame(
  fundamental_score = rnorm(100, 50, 10)
)

# Create STRONG relationship (high R²) but in a context where errors are too big
...
cat("- Conclusion: High R² (", round(stock_r2, 3), ") but ±$", round(stock_rse, 1), "\n")
