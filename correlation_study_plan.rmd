...
```{r level5-medical-confounding}
# Medical research: Treatment effectiveness controlling for baseline health
set.seed(909)

medical_data <- tibble(
  patient_id = 1:120,
  # Baseline health affects both treatment dose and outcome
  ...
  # Recovery outcome depends on both dose and baseline health
...
p2d <- ggplot(finance_data, aes(x = day, y = return, color = stock)) +
  geom_line(size = 1) +
...
- **Performance Evaluation**: Establishing if training programs actually improve outcomes
...
customer_data <- tibble(
  ...
  retention_rate = pmax(0, pmin(1, retention_rate))  # bound between 0 and 1
)
...
```{r level2-business-applications}
...
clinical_data <- tibble(
  ...
  recovery_days = pmax(1, recovery_days)  # minimum 1 day
)
...
- **Medical Research**: Handling skewed biomarkers or ranked outcomes
...
# Combine plots
correlation_methods <- (p9a | p9b | p9c)
correlation_methods + plot_annotation(
  title = "Choosing the Right Correlation Method",
  subtitle = "Pearson vs Spearman vs Kendall for different data types"
)
```

```{r level4-medical-example}
...
# Create correlated variables with a confound
set.seed(123)
z <- rnorm(100)  # confounding variable
x <- 0.5 * z + rnorm(100, 0, 0.7)
y <- 0.7 * z + rnorm(100, 0, 0.7)

# Compare correlations
cor(x, y)  # includes confound influence
pcor(data.frame(x, y, z))$estimate[1,2]  # partial correlation
spcor(data.frame(x, y, z))$estimate[1,2] # semi-partial correlation
```
...
portfolio_long <- portfolio_data %>%
  pivot_longer(cols = -day, names_to = "stock", values_to = "return") %>%
  group_by(stock) %>%
...
p17 <- ggcorrplot(gene_cor,
                  ...
                  subtitle = "Clustering reveals co-expressed pathway groups") +
...
p20 <- ggplot(hospital_data, aes(x = severity, y = recovery)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "black", size = 1.5) +  # overall trend
...
```{r level7-curvilinear-relationships}
...
```{r level8-practice}
library(WRS2)  # robust statistics
...
# Compare methods
cor(x, y)                    # regular Pearson
wincor(x, y)$cor             # Winsorized correlation
...
boot_result <- boot(data.frame(x, y), boot_cor, R = 1000)
boot.ci(boot_result, type = "perc")

# Permutation test
perm_test_result <- corrob(x, y, nboot = 1000)
perm_test_result
```
...
```{r practice-project-3, eval=FALSE}
...
```{r practice-project-5, eval=FALSE}
...
```{r practice-project-6, eval=FALSE}
...
```{r practice-project-8, eval=FALSE}
