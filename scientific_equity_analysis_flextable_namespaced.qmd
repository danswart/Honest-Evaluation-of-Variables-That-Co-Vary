---
title: "Ideology Masquerading as Scientific Inquiry"
...
education_data |>
  dplyr::group_by(every_possible_subgroup) |>
  dplyr::summarize(test_scores = stats::t.test(score ~ treatment)) |>
...
hypothesis_test <- new_education_data |>
  test_specific_hypothesis(
    ...
    controls = base::c("family attitudes toward education", "family_structure", "prior_achievement"),
    pre_registered = TRUE
  )
...
> "For example, one way for school systems to support efforts towards so-called 'Equity'  in student results is to frequently examine any differences in results among student subgroups (Chapter 3)." [@bovee2024data]
...
I have yet to run into anyone in the K-12 delivery system in Texas that has even heard of the website or its study review requirements.  Certainly, the claims regarding 'so-called 'Equity' ', 'diversity' or 'school board affects on student achievement' will never be found there.  Those types of 'studies' would not even qualify for review.
...
## How Skilled Scientists Investigate Claims
...
A scientific investigation would generate multiple testable hypotheses:
...
ft <- flextable::flextable(hypotheses_data) |>
  
  flextable::add_header_lines(values = "Multiple Competing Hypotheses for Observed Group Differences") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
Current so-called 'Equity'  research fails to address whether compared groups are actually comparable.
:::
...
confounding_vars <- base::data.frame(
  ...
  Measurement = base::c("Family income, parent education", "Two-parent vs single-parent", 
                  ...
                  "Friends' academic performance", "Teacher quality, resources"),
  Likely_Impact = base::c("High", "High", "High", "Medium", "High", "High", "High", "Medium", "High")
)


flextable::flextable(confounding_vars) |>
   flextable::set_header_labels(
    Variable = "Variable",
    Measurement = "Measurement",
    Likely_Impact = "Likely Impact"
  ) |>
flextable::add_header_lines(values = "Confounding Variables Often Ignored in So-Called 'Equity' Research") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = ~ Likely_Impact == "High", j = "Likely_Impact", bg = "#ffcccc") |>
  flextable::bg(i = ~ Likely_Impact == "Medium", j = "Likely_Impact", bg = "#ffffcc") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
- Schools with similar demographics but different "so-called 'Equity' " interventions
...
Differences will **ALWAYS** exist between any groups measured at any time, even between the same individuals tested at different times ON THE SAME DAY.
:::
...
# Create random groups - with distinct systematic differences
group_assignment <- base::sample(base::c("Red", "Green", "Blue"), n_students, replace = TRUE)

# Assign distinct means to each group
final_scores <- base::numeric(n_students)
final_scores[group_assignment == "Red"] <- stats::rnorm(base::sum(group_assignment == "Red"), mean = 800, sd = 80)
final_scores[group_assignment == "Green"] <- stats::rnorm(base::sum(group_assignment == "Green"), mean = 1100, sd = 80)
final_scores[group_assignment == "Blue"] <- stats::rnorm(base::sum(group_assignment == "Blue"), mean = 1400, sd = 80)
...
group_means <- demo_data |> 
  ...
  dplyr::summarise(mean_score = base::mean(Score), .groups = "drop")

# Create visualization - all on one plot
p1 <- ggplot2::ggplot(demo_data, ggplot2::aes(x = Score, fill = Group)) +
  ...
  ggplot2::geom_vline(data = group_means,
             ggplot2::aes(xintercept = mean_score, color = Group), 
             linetype = "dashed", linewidth = 1) +
  ggplot2::scale_color_manual(values = base::c("Red" = "#FF6B6B", "Green" = "#51CF66", "Blue" = "#74C0FC")) +
  ggplot2::labs(
    title = "Statistical Variation in Randomly Assigned Groups",
    subtitle = "Groups WILL DIFFER despite NO systematic treatment differences",
    x = "Test Scores",
    y = "Mean and Spread of Distribution"
  ) +
...
flextable::flextable(summary_stats) |>
  ...
  flextable::colformat_double(j = c("Mean", "SD", "N"), big.mark = ",", digits = 1) |>
flextable::add_header_lines(values = "Group Differences That Exist Due to Random Chance Alone") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
flextable::flextable(methodology_matrix) |>
  flextable::set_header_labels(
    Step  = " Step",
    Experimental_Data = "Experimental_Data",
    Observational_Data = "Observational Data"
  ) |>
  flextable::add_header_lines(values = "Methodology Requirements by Study Type") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
All analytical statistics comparing groups rely on the mathematical requirement that **within-group variance should be small relative to between-group variance**. Current so-called 'Equity'  research systematically violates this assumption, rendering the statistical results mathematically meaningless.
:::
...
# Example: Current educational so-called 'Equity'  research
base::set.seed(123)
...
flextable::flextable(variance_comparison) |>
  flextable::colformat_double(j = "Value", big.mark = ",", digits = 1) |> 
  flextable::add_header_lines(values = "The Within-Group vs Between-Group Variance Problem") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
...
# Create problem with heterogeneity within the group

# "Red students" category might include:
base::set.seed(456)
high_achieving_subgroup <- stats::rnorm(50, mean = 1200, sd = 90)    # High-achieving
middle_class_subgroup <- stats::rnorm(100, mean = 900, sd = 100)     # Middle-class (moved to center, wider spread)
disadvantaged_subgroup <- stats::rnorm(75, mean = 700, sd = 100)      # Low-resourced

# Create problem with heterogenity within the group
...
# Add combined group for fourth facet
combined_demo <- base::data.frame(
  Subgroup = base::rep("All Red Students Combined", base::length(all_red_students)),
  Score = all_red_students,
  MainGroup = "Red Students"
)

# Bind together
plot_data <- dplyr::bind_rows(heterogeneous_demo, combined_demo) |>
  dplyr::mutate(Subgroup = base::factor(Subgroup, levels = c(
    "All Red Students Combined",
    "High-achieving Red", 
    "Middle-class Red", 
    "Disadvantaged Red"
  )))
...
p3 <- ggplot2::ggplot(plot_data, ggplot2::aes(x = Score, fill = Subgroup)) +
  ggplot2::geom_density(alpha = 0.7) +
  ggplot2::facet_wrap(~Subgroup, ncol = 2, scales = "free_y") +
  ggplot2::scale_fill_manual(values = base::c(
    "All Red Students Combined" = "#CC0000",
    "High-achieving Red" = "#FF9999", 
    "Middle-class Red" = "#FF6666", 
    "Disadvantaged Red" = "#FF3333"
  )) +
  ggplot2::scale_y_continuous(labels = NULL) +
  ggplot2::labs(
    title = "The 'Red Students' Category Contains Multiple Distinct Populations",
...
flextable::flextable(variance_breakdown) |>
  flextable::colformat_double(j = c("Mean", "Variance"), big.mark = ",", digits = 1) |>
  flextable::add_header_lines(values = "Why Disaggregation Matters: Huge Variance WITHIN 'Red' Students as a Whole") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = ~ Variance > 50000, j = "Variance", bg = "#FFB6C1") |>
  flextable::bg(i = ~ Variance > 10000 & Variance <= 50000, j = "Variance", bg = "#FFFF99") |>
  flextable::bg(i = ~ Variance <= 10000, j = "Variance", bg = "lightblue") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
```
...
p4 <- ggplot2::ggplot(whi_demo, ggplot2::aes(x = Age, y = HRT_Effect, color = Age_Group)) +
  ...
  ggplot2::labs(title = "Women's Health Initiative: Why Age Disaggregation Mattered",
       ...
       x = "Age",
       y = "Below Zero = HRT Bad______________Above Zero = HRT Good,",
       color = "Age Group") +
...
whi_summary <- whi_demo |>
  ...
  dplyr::summarize(
    ...
    N_Ages = dplyr::n(),
    Conclusion = base::ifelse(base::mean(HRT_Effect) > 0, "HRT Beneficial", "HRT Harmful"),
    .groups = 'drop'
  )



flextable::flextable(whi_summary) |>
   flextable::colformat_double(j = c("Mean_Effect"), big.mark = ",", digits = 3) |>
flextable::add_header_lines(values = "HRT Effects by Age Group: Importance of Disaggregation") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
flextable::flextable(homogeneity_results) |>
  flextable::colformat_double(j = c("Levene_Test_p"), big.mark = ",", digits = 3) |>
  flextable::add_header_lines(values = "Homogeneity Tests: Most Demographic Comparisons Are Invalid") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = ~ Groups_Homogeneous == "Yes", j = "Groups_Homogeneous", bg = "yellow") |>
  flextable::bg(i = ~ Valid_Comparison == "Yes", j = "Valid_Comparison", bg = "yellow") |>
  flextable::bg(i = ~ Valid_Comparison == "No", j = "Valid_Comparison", bg = "#FFB6C1") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
  
```

Before any group comparison, test for equal variances. If p < 0.05, groups are not homogeneous and comparisons are invalid.
...
family_effect <- base::ifelse(student_data$family_structure == "Two-parent", 50, -30)
...
flextable::flextable(variance_decomp) |>
  flextable::colformat_double(j = c("Variance"), big.mark = ",", digits = 0) |>
  flextable::colformat_double(j = c("Percentage"), big.mark = ",", digits = 0, suffix = "%") |>
  flextable::add_header_lines(values = "Typical Variance Decomposition in Educational Data") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
Current so-called 'Equity'  research fails at the most basic level - it doesn't establish that comparison groups are actually comparable. The fundamental flaw is treating government-defined demographic categories as if they represent homogeneous groups.
:::
...
flextable::flextable(within_group_demo) |>
  flextable::add_header_lines(values = "Hidden Heterogeneity Within Government-Defined Categories") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::color(i = 1:4, color = "firebrick", part = "body") |> 
  flextable::color(i = 5:8, color = "#004d00", part = "body") |> 
  flextable::color(i = 9:12, color = "dodgerblue", part = "body") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
doctor_data <- base::data.frame(
  gender = base::sample(base::c("Male", "Female"), n_doctors, replace = TRUE, prob = base::c(0.6, 0.4)),
  specialty = base::sample(base::c("Surgery", "Family Medicine", "Pediatrics", "Radiology"), 
                    n_doctors, replace = TRUE, prob = base::c(0.2, 0.3, 0.25, 0.25)),
  years_experience = base::sample(1:30, n_doctors, replace = TRUE),
  marital_status = base::sample(base::c("Married", "Single"), n_doctors, replace = TRUE, prob = base::c(0.7, 0.3)),
  children = base::sample(0:3, n_doctors, replace = TRUE, prob = base::c(0.3, 0.3, 0.25, 0.15)),
  career_breaks = base::sample(0:5, n_doctors, replace = TRUE, prob = base::c(0.6, 0.2, 0.1, 0.05, 0.03, 0.02))
)
...
doctor_data$career_breaks[female_with_children] <- 
  doctor_data$career_breaks[female_with_children] + 
  stats::rpois(base::sum(female_with_children), 1)
...
overall_comparison <- doctor_data |>
  ...
  dplyr::mutate(Analysis = "Comparison of Groups before Disaggregation")
...
comparable_comparison <- comparable_doctors |>
  ...
  dplyr::mutate(Analysis = "Comparable Groups (No children, No career breaks, Never-Married)")

# Combine results
sowell_demo <- dplyr::bind_rows(overall_comparison, comparable_comparison)

flextable::flextable(sowell_demo) |>
  flextable::colformat_double(j = "Mean_Salary", prefix = "$", big.mark = ",", digits = 0) |>
  flextable::add_header_lines(values = "Disaggregate Until Groups are Truly Comparable") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = 1:2, j = 1, bg = "lightblue", part = "body") |>
  flextable::bg(i = 3:4, j = 1, bg = "yellow", part = "body") |>
  flextable::bg(i = 2, j = 2, bg = "lightblue", part = "body") |>
  flextable::bg(i = 3, j = 2, bg = "yellow", part = "body") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
# Visualization
p6 <- doctor_data |>
  dplyr::mutate(comparable_group = base::ifelse(children == 0 & career_breaks == 0 & marital_status == "Single", 
...
flextable::flextable(high_performers) |>
  flextable::add_header_lines(values = "For High Performers Family Structure Matters\n(among other things)") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::color(i = 1:3, color = "firebrick", part = "body") |> 
  flextable::color(i = 4:6, color = "#004d00", part = "body") |> 
  flextable::color(i = 7:9, color = "blue", part = "body") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
correlation_data <- student_data |>
  ...
  dplyr::mutate(
    ...
    family_numeric = base::ifelse(family_structure == "Two-parent", 1, 0),
    education_numeric = dplyr::case_when(parent_education == "College+" ~ 3, 
                                 parent_education == "High School" ~ 2, 
                                 parent_education == "< High School" ~ 1),
    demo_red = base::ifelse(demographic_group == "Red", 1, 0),
    demo_green = base::ifelse(demographic_group == "Green", 1, 0),
    demo_blue = base::ifelse(demographic_group == "Blue", 1, 0)
  ) |>
...
flextable::flextable(correlation_table) |>
  flextable::add_header_lines(values = "Some Predictors For Academic Success\n(among other things)") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = ~ Importance == "High", j = "Importance", bg = "yellow") |>
  flextable::bg(i = ~ Importance == "Medium", j = "Importance", bg = "lightblue") |>
  flextable::bg(i = ~ Importance == "Low", j = "Importance", bg = "#FFB6C1") |>
  flextable::bg(i = 1, bg = "yellow", part = "body") |> 
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
Instead of proper methodology, current "so-called 'Equity'  research" follows this pattern:
...
pseudo_science_steps <- base::data.frame(
  ...
  Validity = base::c("Valid", "Valid", "Valid", "Valid"
                     )
)

flextable::flextable(pseudo_science_steps) |>
  flextable::add_header_lines(values = "Current Practice vs. Scientific Approaches to Educational Equity") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = ~ Validity == "Invalid", j = "Validity", bg = "#FFB6C1") |>
  flextable::bg(i = ~ Validity.1 == "Valid", j = "Validity.1", bg = "yellow") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
pharma_analogy <- base::data.frame(
  Invalid_Practice = base::c(
    "Assume intervention works without testing",
    "Only test on people likely to improve anyway",
    "Ignore side effects",
    "Skip control groups",
    "Get FDA approval based on 'so-called 'Equity' ' rather than efficacy"
  ),
  Educational_Equivalent = base::c(
    "Assume interventions work without testing",
    "Only report favorable results",
    "Ignore negative consequences for populations",
    "No comparison groups or baseline measurements",
  ...
  Consequence = base::c(
    ...
    "Unrecognized harm to populations",
    "Cannot determine causation",
    "Public health catastrophe"
  )
)

flextable::flextable(pharma_analogy) |>
  flextable::add_header_lines(values = "Medical Research Standards vs. Educational Equity Research Standards") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(j = 3, bg = "yellow", part = "body") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
checklist_before <- base::data.frame(
  Requirement = base::c(
    ...
    "Specify meaningful effect sizes", 
    "Identify confounds",
  ...
  Question_to_Ask = base::c(
    ...
    "In your specific case, how large a difference would be meaningful?",
...
flextable::flextable(checklist_before) |>
  flextable::set_header_labels(
    Requirement = "Requirement",
    Question_to_Ask = "Questions to Ask",
    Status = "Status",
    Interpretation = "Interpretation"
  ) |>
  flextable::add_header_lines(values = "Pre-Analysis Requirements") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = ~ Status == "Required", j = "Status", bg = "yellow", part = "body") |>
  flextable::bg(i = 1:5, j = "Requirement", bg = "yellow", part = "body") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
analysis_requirements <- base::data.frame(
  ...
  Interpretation = base::c(
    ...
    "Demographic categories more complex than skin color", 
    "Greatest variation must be between, not within groups"
  )
)

flextable::flextable(analysis_requirements) |>
  flextable::set_header_labels(
    Check = "Check",
    Method = "Method",
    Interpretation = "Interpretation"
  ) |>
  flextable::add_header_lines(values = "Analysis Requirements for Valid Group Comparisons") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = 1:4, j = 3, bg = "yellow", part = "body") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
red_flags <- base::data.frame(
  Question = base::c(
    ...
    "Am I grouping truly comparable individuals or just demographic categories?",
    "Is most of the observed variation within groups or between groups?"
  ),
...
flextable::flextable(red_flags) |>
   flextable::set_header_labels(
    Question = "Question",
    Good_Answer = "Good Answer",
    Warning_Sign = "Warning Signs"
  ) |>
  flextable::add_header_lines(values = "Analysis Requirements for Valid Group Comparisons") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  ...
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::bg(i = 1:6, j = 3, bg = "pink", part = "body") |>
  flextable::bg(i = 1:6, j = 2, bg = "yellow", part = "body") |>
  flextable::bold(i = 1:6, j = 1, part = "body") |>
  ftExtra::colformat_md() |> 
  flextable::autofit()
...
The current approach to educational so-called 'Equity'  research violates fundamental principles of scientific investigation:
...
violations <- base::data.frame(
  Violation = base::c(
    "Inadequate hypothesis testing of core assumptions",
    "Inadequate control for confounding variables", 
    "Inadequate or missing control groups",
    "Inadequate accounting for natural variation",
    "Inadequate replication requirements",
    "Policy implementation without evidence",
    "Inadequate rational subgrouping"
  ),
  Consequence = base::c(
    ...
    "Harm to subjects from ineffective policies",
    "Comparing apples to oranges"
  ),
  Scientific_Standard = base::c(
    "Test competing hypotheses",
    "Control for alternative explanations",
    "Use properly matched comparison groups",
    "Calculate confidence intervals", 
    "Independent replication required",
    "Evidence required before implementation",
    "Disaggregate until individuals within groups are comparable"
  )
)
