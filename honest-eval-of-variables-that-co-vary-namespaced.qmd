...
p1 <- ggplot2::ggplot(df, ggplot2::aes(height_inches, weight_pounds)) +
  ggplot2::geom_point(color = "blue", size = 3) +
  ggplot2::labs(
...
base::set.seed(123)
n <- 100
z <- stats::rnorm(n)  # The variable influencing both x and y (the control variable)
x <- 0.2 * z + stats::rnorm(n, 0, 0.7)  # X is statistical influenced directly by Z with weight 0.6 + random noise,
y <- 0.5 * z + 0.3 * x + stats::rnorm(n, 0, 0.7)  # Y is statistical influenced by both Z directly (.5) and X directly (.3) plus Z through x indirectly, plus random noise,
...
# label outputs
base::cat("Regular correlation of Sleep and Test Score:", base::round(cor_xy, 3), "\n")
base::cat("Regular correlation of Study Time and Test Score:", base::round(cor_Zy, 3), "\n")
base::cat("Partial correlation of Sleep and Test Score (controlling for statistical influence of Study Time):", base::round(pcor_result$estimate, 3), "\n")
...
model1 <- stats::lm(y ~ x, data = data)

# Multiple regression (controlling for Z)
model2 <- stats::lm(y ~ x + z, data = data)
...
p3 <- ggplot2::ggplot(data, ggplot2::aes(x = x, y = y, color = z)) +
  ...
  ggplot2::labs(
    title = "Regression Residual Analysis with Statistical Influence of Z Removed",
    subtitle = base::paste0("Correlation = ", stats1$correlation, " | Model improves predictions by ", stats1$r2 * 100, "%"),
    x = "Sleep Time (X)",
    y = "Test Score (Y)"
  ) +
  ggplot2::theme_minimal(base_size = 16)
...
base::cat("Simple Regression - Sleep coefficient:", stats1$slope, "| R² =", stats1$r2, "| MAE = ±", stats1$mae, "\n")

base::cat("Multiple Regression - Sleep coefficient:", stats2$slope, "(controlling for Study Time) | R² =", stats2$r2, "| MAE = ±", stats2$mae, "\n")

base::cat("Simple Regression - Sleep coefficient:", stats1$correlation, "| R² =", stats1$r2 * 100, "| MAE = ±", stats1$mae, "\n")
...
ggplot2::ggplot(res_df, ggplot2::aes(x_resid, y_resid)) +
  ...
  ggplot2::labs(
    title = base::paste("Regression on Residuals = Partial Correlation (controlling for z) =",
